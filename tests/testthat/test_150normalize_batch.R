...
testing <- normalize_se(pombe_se, filter = TRUE, batch = "limma")
...
testing <-  normalize(pombe_se, filter = TRUE, batch = "svaseq", num_surrogates = 1)
test_counts <- assay(testing)
actual <- as.numeric(test_counts[1:10, 1])
...
testing <-  normalize(pombe_se, filter = TRUE, batch = "fsva", num_surrogates = 1)
test_counts <- assay(testing)
actual <- as.numeric(test_counts[1:10, 1])
...
testing <-  sm(normalize(pombe_se, filter = TRUE, batch = "ssva"))
test_counts <- assay(testing)
actual <- as.numeric(test_counts[1:10, 1])
...
testing <-  normalize(pombe_se, filter = TRUE, batch = "ruvg", num_surrogates = 1, thresh = 1)
test_counts <- assay(testing)
actual <- as.numeric(test_counts[1:10, 1])
...
testing <-  normalize(pombe_se, filter = TRUE, batch = "ruv")
test_counts <- assay(testing)
actual <- as.numeric(test_counts[1:10, 1])
...
##testing <-  normalize(pombe_se, filter = TRUE, batch = "isva")
...
testing <-  normalize(pombe_se, filter = TRUE, batch = "combat")
test_counts <- assay(testing)
actual <- as.numeric(test_counts[1:10, 1])
...
pombe_result <- compare_surrogate_estimates(pombe_se, do_catplots = TRUE)
...
pombe_filt <- normalize(pombe_se, filter = TRUE)
...
prev_counts <- counts_from_surrogates(pombe_filt, adjust = ruv_model, method = "cbcb_subtract")
