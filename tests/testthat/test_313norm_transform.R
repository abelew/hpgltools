...
pasilla_trans <- sm(normalize(pasilla_se, transform = "log2"))
...
names(expected) <- test_genes
pasilla_trans <- sm(normalize(pasilla_se, transform = "log10"))
actual_df <- exprs(pasilla_trans)
actual <- actual_df[test_genes, c("untreated1")]
...
names(expected) <- test_genes
pasilla_trans <- sm(normalize(pasilla_se, transform = "log"))
actual_df <- exprs(pasilla_trans)
actual <- actual_df[test_genes, c("untreated1")]
