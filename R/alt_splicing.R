...
#' Collect the outputs from suppa.
#'
#' I invoked suppa in a pretty specific way and the various outputs
#' get specific home directories and suffixes.  This is intended
#' partially to document that for myself and make it easier to find
#' them.
#'
#' @param file_prefix Where do we expect to find the files?
#' @param numerator The numerator and denominator are prefixed to the
#'  output files.
#' @param denominator Ibid.
...
#' Collect the outputs from rMATS.
#'
#' I invoked rMATS in a pretty specific way and the various outputs
#' get specific home directories and suffixes.  This is intended
#' partially to document that for myself and make it easier to find
#' them.
#'
          ...
          no = ifelse(
            ...
            no = ifelse(plotting_data[["plot_cat"]] == "AF",
                        ...
                        no = ifelse(plotting_data[["plot_cat"]] == "AL",
                                    ...
                                    no = "Transcripts")))))))
  plotting_data[["category"]] <- plotting_data[["plot_cat"]]
  insig_idx <- plotting_data[["pvalue"]] > sig_threshold
  plotting_data[insig_idx, "plot_cat"] <- "Insignificant"
  ## If, somehow something is observed as unknown, make it insignificant.
  unknown_idx <- plotting_data[["plot_cat"]] == "Unknown"
  plotting_data[unknown_idx, "plot_cat"] <- "Insignificant"
