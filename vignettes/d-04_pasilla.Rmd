...
expressionset in the following block along with creating an experimental design.  create_se() will
...
## Make sure it is still possible to create an se
pasilla_se <- sm(create_se(count_dataframe = counts, metadata = metadata,
...
pasilla_metrics <- sm(graph_metrics(pasilla_se, ma = TRUE, qq = TRUE))
...
```{r, fig.show = "hide"}
norm <- normalize(pasilla_se, transform = "log2", convert = "cpm", norm = "quant", filter = TRUE)
...
pasilla_pairwise <- all_pairwise(pasilla_se)
pasilla_pairwise
pasilla_tables <- combine_de_tables(pasilla_pairwise, excel = "pasilla_tables.xlsx")
pasilla_tables
pasilla_sig <- extract_significant_genes(pasilla_tables, excel = "pasilla_sig.xlsx")
pasilla_sig
pasilla_ab <- extract_abundant_genes(pasilla_pairwise, excel = "pasilla_abundant.xlsx")
```

```{r}
pasilla_tables[["plots"]][["untreated_vs_treated"]][["deseq_ma_plots"]]
pasilla_tables[["plots"]][["untreated_vs_treated"]][["edger_ma_plots"]]
pasilla_tables[["plots"]][["untreated_vs_treated"]][["limma_ma_plots"]]
```
...
pasilla_length <- fData(pasilla_se)[, c("ensembl_gene_id", "cds_length")]
