...
## Notice that the colors were auto-chosen by create_se() and they should
## be maintained throughout this process
fis_libsize <- plot_libsize(fission_se)
fis_libsize
...
##fis_rawpca <- plot_pca(fission_se, se_names = fission_se$condition, cis = NULL)
plot_pca(fission_se)

## So, normalize the data
norm_se <- normalize(fission_se, transform = "log2", norm = "quant", convert = "cpm")
## And try the pca again
plot_pca(norm_se, plot_labels = "normal", title = "normalized pca", cis = NULL)
...
```{r test_3d}
testing <- plot_pca(norm_se, num_pc = 3)
testing
...
```{r}
normbatch_se <- normalize(fission_se, transform = "log2", norm = "quant",
                            convert = "cpm", batch = "sva")
fis_normbatchpca <- plot_pca(normbatch_se,
...
fis_info <- pca_information(
  norm_se, se_factors = c("condition","batch","strain","minute"), num_components = 6)
...
batchnorm_se <- normalize(fission_se, batch = "sva", filter = TRUE,
                          transform = "log2", convert = "cpm")
plot_pca(batchnorm_se, plot_title = "limma corrected pca")

test_pca <- pca_information(
  batchnorm_se, se_factors = c("condition","batch","strain","minute"), num_components = 6)
```
...
```{r}
fission_boxplot <- plot_boxplot(fission_se)
fission_boxplot

sf_se <- normalize(fission_se, norm = "sf")
fission_boxplot <- plot_boxplot(sf_se)
fission_boxplot

tm_se <- normalize(fission_se, norm = "tmm")
fission_boxplot <- plot_boxplot(tm_se)
fission_boxplot

rle_se <- normalize(fission_se, norm = "rle")
fission_boxplot <- plot_boxplot(rle_se)
fission_boxplot

up_se <- normalize(fission_se, norm = "upperquartile")
fission_boxplot <- plot_boxplot(up_se)
fission_boxplot

compare_12 <- plot_single_qq(fission_se, x = 1, y = 2)
compare_12$log
```
...
```{r}
plot_corheat(norm_se)

plot_corheat(batchnorm_se)

plot_disheat(norm_se)

plot_disheat(batchnorm_se)
```
...
```{r}
test_varpart <- simple_varpart(fission_se)
...
test_varpart <- simple_varpart(fission_se, fstring = "~ strain + minute + replicate")
test_varpart[["percent_plot"]]
test_varpart[["partition_plot"]]
```
